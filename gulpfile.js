...
gulp.task(
  ...
  gulp.series("build_modules", "copy_ts", () => {
    return Promise.all(
      components.map(name => {
        Promise.all([
          gulp.src("styles/index.js").pipe(gulp.dest(`lib/${name}/style`)),
          gulp
            .src(`styles/${name}.scss`)
            .pipe(
              replace(/(\/\*\*Variable\*\*\/)/, `@import './variable.scss';\n`)
            )
            .pipe(sass())
            .pipe(
              prefixer({
...
gulp.task(
  "build_clean",
  gulp.series("build_style", "lib_rename", () => {
    return gulp.src(["lib/demo.html"]).pipe(clean());
  })
);
